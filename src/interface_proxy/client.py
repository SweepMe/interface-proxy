...
class RunServer:
    ...
    def __new__(cls, _: object | int, server_exe: Path | None = None, server_py: Path | None = None):
        obj = cls._instances.get(server_exe, None) or cls._instances.get(server_py, None)
        if not obj:
            obj = super().__new__(cls)
        if cls._instances.get(server_exe, obj) != cls._instances.get(server_py, obj):
            msg = ("Mismatch between exe and py version. Either the exe does not belong to the py version, or"
                   "RunServer was previously called with different arguments.")
            raise ValueError(msg)

        if server_exe:
            cls._instances[server_exe] = obj
        if server_py:
            cls._instances[server_py] = obj
